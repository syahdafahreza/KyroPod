...
<head>
    ...
    <link rel="icon" type="image/svg+xml" href="/svg/compact-disc-solid-light.svg" media="(prefers-color-scheme: dark)">
    <link rel="icon" type="image/svg+xml" href="/svg/compact-disc-solid-dark.svg" media="(prefers-color-scheme: light)">
                ...
                const checkOverflowAndSetMarquee = () => {
                    if (targetRef.current) {
                        ...
                        if (overflows) {
                            ...
                            targetRef.current.style.setProperty('--marquee-duration', `${duration}s`);
                            targetRef.current.style.setProperty('--marquee-distance', `-${distanceToScroll}px`);
                        } else {
        ...
        function App() {
            ...
            useEffect(() => {
                ...
                return () => {
                    ...
                    if (lowShelfFilterRef.current) lowShelfFilterRef.current.disconnect();
                    if (peakingFilterRef.current) peakingFilterRef.current.disconnect();
                    if (highShelfFilterRef.current) highShelfFilterRef.current.disconnect();

                    // It's generally not recommended to close the AudioContext if other parts of your app might use it,
                    // but for a single player, it might be acceptable. Consider disconnecting nodes instead of closing.
                    // if (audioContextRef.current && audioContext.state !== 'closed') {
            ...
            const playNextSong = useCallback(() => {
                ...
                if (isRepeating && currentSongIndex !== null) {
                    audio.currentTime = 0;
                    audio.play().catch(e => console.error("Error playing repeated song:", e));
                    return;
                }
            ...
            useEffect(() => {
                ...
                // Volume is now controlled by the gainNode, so remove direct audio.volume assignment here
                // audio.volume = volume;
            ...
            useEffect(() => {
                ...
                if (currentSongIndex !== null && playlist.length > 0 && playlist[currentSongIndex]) {
                    ...
                    if (window.jsmediatags) {
                        window.jsmediatags.read(song, {
                            onSuccess: function (tag) {
                                setSongTitle(tag.tags.title || song.name.split('.').slice(0, -1).join('.') || "Unknown Title");
                                ...
                                if (tag.tags.picture) {
                                    ...
                                    setAlbumArtUrl(`data:${format};base64,${window.btoa(base64String)}`);
                                } else {
                            ...
                            onError: function (error) {
                                ...
                                setSongTitle(song.name.split('.').slice(0, -1).join('.') || "Unknown Title");
                    ...
                    if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
                        audioContextRef.current.resume().catch(e => console.error("Error resuming AudioContext on song change:", e));
                    }

                    if (isPlaying) {
                        audio.play().catch(error => {
                            console.error("Error playing song after index change:", error);
                            setIsPlaying(false);
                        });
                    }
                } else {
            ...
            }, [currentSongIndex, playlist, isPlaying]); // isPlaying is still a dependency here because it dictates whether to attempt play
            ...
            useEffect(() => {
                ...
                if (isPlaying && audioContextRef.current && audioContextRef.current.state === 'suspended') {
                    audioContextRef.current.resume().catch(e => console.error("Error resuming AudioContext on play:", e));
                }

                if (isPlaying) {
                    audio.play().catch(error => {
                        console.error("Autoplay prevented in isPlaying useEffect:", error);
                        setIsPlaying(false);
                    });
                } else {
                    audio.pause();
                    ...
                    if (file) {
                        setPlaylist((prevPlaylist) => {
                            ...
                            if (isShuffling) {
                                ...
                                if (currentSongIndex === null && shuffledFullPlaylist.length === 1) {
                                    setCurrentSongIndex(0);
            ...
            const togglePlayPause = () => {
                ...
                if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
                    audioContextRef.current.resume().catch(e => console.error("Error resuming AudioContext on play/pause:", e));
                }
            ...
            const playSong = (indexInFiltered) => {
                ...
                if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
                    audioContextRef.current.resume().catch(e => console.error("Error resuming AudioContext on playlist item click:", e));
                }
            ...
            const handleDeleteSong = (indexToDeleteInOriginal, event) => {
                ...
                setPlaylist(prevPlaylist => {
                    ...
                    if (newPlaylist.length === 0) {
                        ...
                        newCurrentSongIndex = (indexToDeleteInOriginal >= newPlaylist.length) ? 0 : indexToDeleteInOriginal;
                    } else if (currentSongIndex > indexToDeleteInOriginal) {
            ...
            return (
                <div className="container">
                    ...
                    <div className={`playlist-card ${isSidebarOpen || isEQSidebarOpen ? 'open' : 'collapsed'}`}>
                        <div className="playlist-header">
                            ...
                            <div className="header-buttons-group">
                                ...
                                {isSidebarOpen && (
                                    <button className="search-toggle-btn" onClick={handleSearchClick} aria-label="Search Playlist">
                                ...
                                >
                                    <i className={`fas ${isSidebarOpen || isEQSidebarOpen ? 'fa-times' : 'fa-bars'}`}></i>
                                </button>
                            </div>
                        </div>
                        ...
                        {(!isSidebarOpen && !isEQSidebarOpen) && (
                            <button
                                ...
                                aria-label="Open Equalizer" onClick={toggleEQSidebar}
                            >
                        ...
                        {isSidebarOpen && !isEQSidebarOpen && (
                            <>
                                ...
                                <ul className="playlist-list">
                                    {filteredPlaylist.length === 0 && searchQuery !== '' ? (
                                    ...
                                    ) : filteredPlaylist.length === 0 && playlist.length > 0 && searchQuery === '' ? (
                                        ...
                                        filteredPlaylist.map((song, index) => (
                                            ...
                                            >
                                                <div className="playlist-item-inner" onClick={() => playSong(index)}>
                            ...
                            <div className="eq-controls">
                                <div className="eq-band">
                                    ...
                                    <input
                                        ...
                                        className="eq-slider"
                                    />
                                    <span>{eqSettings.low}dB</span> {/* Changed from bass to low */}
                                </div>
                                <div className="eq-band">
                                    <label>Mid</label>
                                    <input
                                ...
                                <div className="eq-band">
                                    ...
                                    <input
                                        ...
                                        className="eq-slider"
                                    />
                                    <span>{eqSettings.high}dB</span> {/* Changed from treble to high */}
                                </div>
                                <button
                        ...
                        {(!isSidebarOpen && !isEQSidebarOpen) && (
                            <div className="playlist-card-logo mb-3 font-weight-bold">Kyrop
                                <i className="fas fa-compact-disc"></i>d {/* Example logo icon */}
                            </div>
                        )}
                    ...
                    <div className="player-card">
                        <div className="album-art-wrapper">
                            <img src={albumArtUrl} alt="Album Art" className="album-art" onError={(e) => {
                                e.target.onerror = null;
                                e.target.src = 'https://placehold.co/200x200/bec8e4/9baacf?text=No+Art';
                            }} />
                        </div>
                        <div className="song-info">
                            ...
                            >
                                <span>
                                    ...
                                    {isTitleOverflowing && (
                                        <>
                                            <span className="inline-block" style={{ width: `${gapPixels}px`, display: 'inline-block' }}></span>
                                            {songTitle}
                                        </>
                                    )}
                                </span>
